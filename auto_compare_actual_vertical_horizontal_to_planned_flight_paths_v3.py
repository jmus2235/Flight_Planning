...
def create_combined_analysis_plot(planned_df, actual_df, line_number, output_filepath):
    ...
    ax3.set_position([pos3.x0, pos3.y0 - 0.02, pos3.width, pos3.height])  # Adjust -0.02 value to move more/less
    ...
    ax2.set_title(f'Flight Line {line_number} - Vertical Deviation from Planned Path', 
                 fontsize=14, fontweight='bold')
    ...
    ax3.set_title(f'Flight Line {line_number} - Horizontal Deviation from Planned Path', 
                 fontsize=14, fontweight='bold')
...
def process_single_line(line_number, north_time, south_time):
    ...
    try:
        ...
        vert_var, horiz_var = create_combined_analysis_plot(
            planned_df, actual_df_truncated, line_number, output_combined)
