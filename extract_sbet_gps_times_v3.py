...
# Input files/folders
SBET_FOLDER = r"C:\Users\jmusinsky\Documents\Data\NEON Sites\Flight_Boundaries_ArcGIS_Online\D01_HARV\SBET"  # folder with SBET shapefiles
FLIGHTLINE_SHAPEFILE = r"C:\Users\jmusinsky\Documents\Data\NEON Sites\Flight_Boundaries_ArcGIS_Online\D01_HARV\Shapes\D01_HARV_C1_P1_P2_v3_fltlines_copy.shp"

# Output file
OUTPUT_CSV = r"C:\Users\jmusinsky\Documents\Data\TopoFlight\Conversions\pointsToElev\HARV_2025_GPS_start_end_times.csv"
...
def process_flight_line_with_sbet(line_num, flight_line_geom, sbet_gdf, buffer_dist, heading_tol, time_gap_thresh, sbet_filename):
    """
    Process a single flight line against a single SBET file
    ...
    return {
        ...
        'south_dist': south_dist,
        'segment_length': len(longest_segment)
    }

def process_flight_line(line_num, flight_line_geom, sbet_files_dict, buffer_dist, heading_tol, time_gap_thresh):
    """
    Process a single flight line against all SBET files
    Returns the best result or None if no valid data found
    """
    print(f"  Processing line {line_num}...")
    
    # Get the start and end points for display
    line_coords = list(flight_line_geom.coords)
    line_start = Point(line_coords[0])
    line_end = Point(line_coords[-1])
    ...
    # Try each SBET file
    results = []
    for sbet_filename, sbet_gdf in sbet_files_dict.items():
        result = process_flight_line_with_sbet(
            line_num,
            flight_line_geom,
            sbet_gdf,
            buffer_dist,
    ...
    if len(results) == 0:
        print(f"    No valid data found in any SBET file")
        return None
    
    # If multiple SBET files have data for this line, select the best one
    # Criteria: longest segment, then smallest total distance to endpoints
    best_result = max(results, key=lambda x: (x['segment_length'], -x['north_dist'] - x['south_dist']))
    
    if len(results) > 1:
        print(f"    Found data in {len(results)} SBET file(s), selected: {best_result['Shapefile']}")
    else:
        print(f"    Found data in: {best_result['Shapefile']}")
    
    print(f"    North GPS time: {best_result['North']} (distance: {best_result['north_dist']:.2f}m)")
    print(f"    South GPS time: {best_result['South']} (distance: {best_result['south_dist']:.2f}m)")
    print(f"    Segment length: {best_result['segment_length']} points")
    
    # Return only the fields needed for output
    return {
        'Line': best_result['Line'],
        'North': best_result['North'],
        'South': best_result['South'],
        'Shapefile': best_result['Shapefile']
    }
...
def main():
    ...
    # Find all SBET shapefiles in the folder
    print(f"\nSearching for SBET shapefiles in: {SBET_FOLDER}")
    sbet_files = glob.glob(os.path.join(SBET_FOLDER, "*.shp"))
    
    if len(sbet_files) == 0:
        print(f"ERROR: No shapefiles found in {SBET_FOLDER}")
        return
    
    print(f"Found {len(sbet_files)} SBET shapefile(s):")
    for f in sbet_files:
        print(f"  - {os.path.basename(f)}")
    
    # Load all SBET shapefiles
    print("\nLoading SBET files...")
    sbet_files_dict = {}
    for sbet_file in sbet_files:
        sbet_filename = Path(sbet_file).stem
        print(f"  Loading {sbet_filename}...")
        sbet_gdf = gpd.read_file(sbet_file)
        ...
        print(f"    Loaded {len(sbet_gdf)} points")
        sbet_files_dict[sbet_filename] = sbet_gdf
    
    # Load flight lines
    print(f"\nLoading flight line shapefile: {FLIGHTLINE_SHAPEFILE}")
    ...
    if len(results) > 0:
        ...
        # Show which SBET files were used
        print(f"\nSBET files used:")
        for sbet_file in output_df['Shapefile'].unique():
            count = len(output_df[output_df['Shapefile'] == sbet_file])
            print(f"  {sbet_file}: {count} lines")
    else:
        print(f"\n{'=' * 70}")
